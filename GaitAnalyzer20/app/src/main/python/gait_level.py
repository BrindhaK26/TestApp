def main(path,imgPath):
    ...
    capID = 0
    p=time.time()
    cap = cv2.VideoCapture(path)  # Number According to your Camera
    Distance_level = 0
    travedDistance = 0
    changeDistance = 0
    velocity = 0
    ...
    while (cap.isOpened()):
        ...
        for (face_x, face_y, face_w, face_h) in Faces:
            if face_width_in_frame != 0:
                ...
                if(abs(p-time.time())>=6):
                    if(abs(prev-averageSpeed)<=1.0):
                        #print("gait-level-surface-points:0")

                        cap.release()
                        return 0
                    elif(abs(prev-averageSpeed)>=2.5):
                        #print("gait-level-surface-points:1")

                        cap.release()
                        return 1
                    elif(abs(prev-averageSpeed)>=3.5):
                        #print("gait-level-surface-points:2")

                        cap.release()
                        return 2
                    elif(abs(prev-averageSpeed)>=4.5):
                        #print("gait-level-surface-points:3")

                        cap.release()
                        return 3
                prev = averageSpeed
                #cv2.putText(frame, f"Speed: {round(averageSpeed,2)} m/s",(30, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2,cv2.LINE_AA)
                #cv2.putText(frame, f"Distance {roundedDistance} meter", (face_x-6, face_y-6), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2,   cv2.LINE_AA)
